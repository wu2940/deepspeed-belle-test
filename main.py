...
def parse_args():
    ...
    parser.add_argument('--use_auth_token', type=str, default=os.getenv('HF_AUTH_TOKEN', None), help='Hugging Face token for private models')
...
def main():
    ...
    print("args.use_auth_token=", args.use_auth_token)

    tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, use_auth_token=args.use_auth_token, trust_remote_code=True)
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, use_auth_token=args.use_auth_token, ignore_mismatched_sizes=True, trust_remote_code=True)
    model.to(device)
